...
<?php 
    ...
    if($this->session->flashdata('reply_insert') == 'success') {
        echo "demo.showNotification('top','center','check',2,'subject Added.');";
    }
...
<?php 
    ...
    if($this->session->flashdata('reply_edit') == 'success')
        echo "setTimeout(function() {
            demo.showNotification('top','center','check',2,'subject Updated.');
        },500)";
?>
...
function selectClass(classesID) {
    ...
    $.ajax(
    {
        ...
        success: function(result) {
            if(result == 'nosubject')
                $('#tbody')[0].innerHTML = '<tr><td colspan="6"><center>No subject Found</center></td></tr>';
            else {
                datatableDestroy();
...
function clear_edit_form() {
        $('#supportModal').find('#modal-body').find('#subject_name_edit').hide();
          ...
          $.ajax(
          {
                 ...
                 { 
                    if(msg=='fail'){
                        ...
                        datatableSet();
                        demo.showNotification('top','center', 'done',2, 'subject Deleted..');
                    }
...
function getEdit(subjectID) {

    var data = "subjectID="+subjectID;
    var infoModal = $('#supportModal');
    $.ajax({
        type: "post",
        url: "<?= base_url();?>subject/editForm",
        data: data,
        success: function(result) {
...
function getEditReopen(subjectID) {
